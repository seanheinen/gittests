;(function(window, angular, undefined) {
  ...
  var mfb = angular.module('ng-mfb', ['ionic']);

  mfb.run(['$templateCache', function($templateCache) {
    $templateCache.put('ng-mfb-menu-default.tpl.html',
      '<ul class="mfb-component--{{position}} mfb-{{effect}} floating-action"' +
    ...
    // custom one button - sean heinen
    $templateCache.put('ng-mfb-menu-btn.tpl.html',    
      '<ul class="mfb-component--{{position}} mfb-{{effect}} floating-action"' +
      '    data-mfb-toggle="{{togglingMethod}}" data-mfb-state="{{menuState}}">' +
      '  <li class="mfb-component__wrap">' +
      '    <a ng-click="oneButtonClicked()"' +
      '       ng-attr-data-mfb-label="{{label}}" class="mfb-component__button--main">' +      
      '     <i class="one-button-icon ion-refresh"></i>' +
      '    </a>' +
      '</li>' +
      '</ul>'
    );
    ...
    // custom one button - sean heinen
    $templateCache.put('ng-mfb-upload.tpl.html',    
      '<ul class="mfb-component--{{position}} mfb-{{effect}} floating-action"' +
      '    data-mfb-toggle="{{togglingMethod}}" data-mfb-state="{{menuState}}">' +
      '  <li class="mfb-component__wrap">' +
      '    <a ng-click="oneButtonClicked()"' +
      '       ng-attr-data-mfb-label="{{label}}" class="mfb-component__button--main">' +      
      '     <i class="one-button-icon ion-upload"></i>' +
      '    </a>' +
      '</li>' +
      '</ul>'
    );
  ...
  mfb.directive('mfbMenu', ['$timeout', function($timeout) {
    return {
      ...
      controller: ['$scope', '$attrs', '$ionicBackdrop', function($scope, $attrs) {
        var openState = 'open',
          closedState = 'closed';
        ...
        $scope.oneButtonClicked = oneButtonClicked; // sean edits
        ...
        function open() {    
          ...
          //$scope.backdrop = angular.element('<div class="action-backdrop backdrop visible active"></div>');
          //$attrs.$$element.after($scope.backdrop);
