...
angular.module('epicmobile.services', ['ionic', 'ngCordova', 'base64'])
  ...
  *************************************************************************************************************************************************/
   ...
   * Logout
  *************************************************************************************************************************************************/
  .factory("$smp", ['$state', '$ionicLoading', '$timeout', function($state, $ionicLoading, $timeout){
    ...
    SMP.login = function(loginSuccess, loginError){
      ...
      function customShowNotification(screenId, notificationKey, notificationMessage) {
        if (screenId == "SCR_SSOPIN_SET" || screenId == "SCR_UNLOCK" || screenId == "SCR_REGISTRATION" || screenId == "SCR_SET_PASSCODE_MANDATORY" || screenId == "SCR_SET_PASSCODE_OPT_ON" || screenId == "SCR_SET_PASSCODE_OPT_OFF" || screenId == "SCR_ENTER_AFARIA_CREDENTIAL" || screenId == "SCR_ENTER_CREDENTIALS") {                                         
          ...
          // check why smp failed
          if(notificationKey === "ERR_REG_FAILED_UNATHORIZED"){
            //locked out
          }
      ...
      // init call backs
      function removeStore(result) 
      { 
        sap.Logon.core.deleteRegistration(authLogin, authLogin);
      }

      function authLogin(response)  { 
        sap.Logon.init(cbsuccess, cberror, localStorage.getItem("ApplicationID"), connectionSettings, logonView);// LogonController line 476        
      }
      ...
      // call login init, then delete registration, then init again:
